...
which defined in Microsoft.SCP.Net.SDK\sdk\Microsoft.SCPNet.dll.config.
...
E.g. SCP.NET SDK package uses Newtonsoft.Json 6.0.8, but if one of your dependency requires Newtonsoft.Json 4.5.0 or above, 
you can just add following assembly binding redirections to solve this conflict:
	<runtime>
...
BUG FIXES:
1. Reference "Newtonsoft.Json" nuget package explicitly in SCP.NET SDK package simpliying your project references. 
We also tried to reference "log4net" and "ZooKeeper.Net" nuget package explicitly, however the "ZooKeeper.Net" nuget package only can work with log4net 1.2.10 for now.
We have contacted with the owner of "ZooKeeper.Net" nuget package, and will solve this problem in the future.
2. SCPAPI returning HttpStatus 502 on TopologySubmit is now fixed in HDInsight Storm Cluster Version: 3.2.4.583 or above

KNOWN ISSUES:
1. SCPAPI on HDInsight can return HttpStatus 404 if your topology package size is larger than 30 MB. You can workaround this problem by modifying its Web.config.
...
3. Updating log4net NuGet version from 1.2.10 to 2.0.0 or above can cause assembly load failures leading to SCP.Net bolt or spout task crashes. If you updated the log4net to 2.0.3 (1.2.13) and are seeing these issues, please revert to log4net 1.2.10.
